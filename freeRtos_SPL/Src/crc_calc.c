...
//static unsigned short Reflect(unsigned short v, int b);

unsigned short Poly = 0x8005;
const unsigned short Table[] = { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0,
...
//Расчет CRC16 производится с помощью функции CalcCRC:
unsigned short CalcCRC(unsigned char *Tlg, unsigned int N) {
	unsigned short crc = 0x0000;
	while (N--)
		crc = Table[(crc ^ *Tlg++) & 0xff] ^ (crc >> 8);
	return crc;
}
