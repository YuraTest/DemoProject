...
BaseType_t SDIO_scanFiles(void)
{
	...
	if (_fresult == FR_OK) 
	{
		for (; i < SDIO_FILES_TO_VIEW_MAX; i++) 
		{
			...
			if (_fresult != FR_OK || _fileInfo.fname[0] == 0x00) break;  /* Break on error or end of dir */
			if (_fileInfo.fname[0] == '.') continue;             /* Ignore dot entry */

			_fileInfo.lfname = _lfName;
			_fileInfo.lfsize = sizeof _lfName;

			#if _USE_LFN
				if(*_fileInfo.lfname) strcpy(SDIO_files[SDIO_filesNum].fileName, _fileInfo.lfname);
				else strcpy(SDIO_files[SDIO_filesNum].fileName, _fileInfo.fname);
			#else
				strcpy(SDIO_files[SDIO_filesNum].fileName, _fileInfo.fname);
			#endif

			SDIO_files[SDIO_filesNum].fileName[(SDIO_FILE_STRUCT_NAME_MAX_LENGTH - 1)] = 0x00;
			SDIO_files[SDIO_filesNum].isDirectory = (_fileInfo.fattrib & AM_DIR);
