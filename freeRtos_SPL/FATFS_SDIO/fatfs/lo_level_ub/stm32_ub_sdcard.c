...
int MMC_disk_read(BYTE *buff, DWORD sector, BYTE count) {
	...
	//SD_ReadMultiBlocks(buff, sector << 9, 512, 1); // Buffer fix
	SD_ReadMultiBlocks(buff, sector * 512, 512, count); // See: http://we.easyelectronics.ru/STM32/programmnyy-dekoder-mp3-perehod-na-platformu-stm32f407.html
...
int MMC_disk_write(const BYTE *buff, DWORD sector, BYTE count) {
	...
	//SD_WriteMultiBlocks((BYTE *)buff, sector << 9, 512, 1); // Buffer fix
	SD_WriteMultiBlocks((BYTE *) buff, sector * 512, 512, count); // See: http://we.easyelectronics.ru/STM32/programmnyy-dekoder-mp3-perehod-na-platformu-stm32f407.html
...
static void NVIC_Configuration(void) {
	...
	NVIC_SetPriority(SD_SDIO_DMA_IRQn, 5);
...
SD_Error SD_WaitReadOperation(void) {
	...
	while ((DMAEndOfTransfer == 0x00) && (TransferEnd == 0)
		...
		timeout--;
#if (USE_SDIO_RTOS_SWAP_TASK_CALLBACK == 1)
		SDIO_onTaskSuspend();
#endif
	}
...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize) {
	...
	SDDMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte; //DMA_MemoryDataSize_Word; // See: http://we.easyelectronics.ru/STM32/programmnyy-dekoder-mp3-perehod-na-platformu-stm32f407.html
